...
namespace Msg.Utils
{
    ...
    public class ExcelHelper
    {
        ...
        public static void DataTable1Excel(System.Data.DataTable dtData)
        {
            ...
            if (dtData != null)
            {
                curContext.Response.AddHeader("content-disposition", "attachment;filename=" + HttpUtility.UrlEncode("订单导出_"+DateTime.Now.ToString("yyyyMMddhhmmss"), System.Text.Encoding.UTF8) + ".xls");
                ...
                foreach (DataRow rows in dtData.Rows)
                {
                    ...
                    {
                        rows[i] = "=\""+rows[i].ToString()+"\"";
                    }

                }
                gvExport = new GridView();
                gvExport.DataSource = dtData.DefaultView;
                //gvExport.Attributes.AddAttributes(htmlWriter);
                gvExport.AllowPaging = false;
                gvExport.DataBind();
                gvExport.RenderControl(htmlWriter);
                curContext.Response.Write("<meta http-equiv=\"Content-Type\"   content=\"text/html;charset=utf-8\"/>" + strWriter.ToString().Replace("&lt;br&gt;", "<br style='mso-data-placement:same-cell;'/> "));
    ...
    {
        private static IList<DataDescription> Description { get; set; }
        /// <summary>
        /// 初始化
        /// </summary>
        ...
        public static void ExportToExcel(object dataList, IList<DataDescription> dataDescription, string title)
        {
            ...
            foreach (var item in dataDescription)
            {
                ...
                bf.DataField = item.Field;
                bf.HeaderText = item.HeadText;
                gvw.Columns.Add(bf);
            }

            SetStype(gvw);
            gvw.DataSource = dataList;
            gvw.RowDataBound += new GridViewRowEventHandler(CustomersGridView_RowDataBound);
            gvw.DataBind();
        ...
        private static void ExportToExcel(GridView gvw, string title)
        {
            ...
            fileName = string.Format("{0}.xls", title);
        ...
        #region gridview绑定事件
        public static void CustomersGridView_RowDataBound(Object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                for (int i = 0; i < Description.Count; i++)
                {
                    ...
                    if (item.IsIntact)
                    {
                        e.Row.Cells[i].Style.Add("vnd.ms-excel.numberformat", "@");
                    }
                }
            }
        }
