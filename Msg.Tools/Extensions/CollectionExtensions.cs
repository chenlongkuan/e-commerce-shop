...
namespace Msg.Tools.Extensions
{
    /// <summary>
    ///     集合扩展方法类
    /// </summary>
    public static class CollectionsExtensions
    {
        ...
        public static IQueryable<TEntity> Where<TEntity, TKey>(this IQueryable<TEntity> source, Expression<Func<TEntity, bool>> predicate, int pageIndex, int pageSize,
            out int total, PropertySortCondition[] sortConditions = null) where TEntity : BaseEntity<TKey>
        {
        ...
        {
            return source.OrderBy(d => Guid.NewGuid());
        }
        ...
        #region IEnumerable和 IQueryable 转化为DataTable

        public static DataTable ConvertToDataTable<TEntity>(this IEnumerable<TEntity> enumerable)
        {
            ...
            foreach (TEntity item in enumerable)
            {
                ...
                foreach (PropertyDescriptor dp in TypeDescriptor.GetProperties(typeof(TEntity)))
                {
